...
unNodePerPos:- position(P), findall(visited-I-P, node(I), Lits), exactly(1, Lits), fail.
unNodePerPos.

unaPosPerNode:- node(I), I =\= 1, findall(visited-I-P, position(P), Lits), exactly(1, Lits), fail.
unaPosPerNode.

adjacent:- position(P), P =\= 20, P1 is P+1, adjacency(I, L), findall(visited-I1-P1, member(I1, L), Lits),
	writeClause([\+visited-I-P | Lits]), fail.
adjacent.

varCost:- position(P1), P2 is P1+1, node(I1), node(I2), mod(I1, 2) =\= mod(I2, 2),
	writeClause([\+visited-I1-P1, \+visited-I2-P2, cost-P1]), fail.
varCost.

limitCost:- maxCost(N), findall(cost-P, position(P), Lits), atMost(N, Lits), fail.
limitCost.
