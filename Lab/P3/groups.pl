...
minMaxGroup:- minGroup(Min), maxGroup(Max), group(G), findall(sg-S-G, student(S), Lits),
	atLeast(Min, Lits), atMost(Max, Lits), fail.
minMaxGroup.

incompatible:- incompatible(X, Y), group(G), writeClause([\+sg-X-G, \+sg-Y-G]), fail.
incompatible.

onegroupPerStudent:- student(S), findall(sg-S-G, group(G), Lits), exactly(1, Lits), fail.
onegroupPerStudent.
