...
nursesPerHora:- hour(H), needed(H, A), findall(worksNH-N-H, nurse(N), Lits), atLeast(A, Lits), fail.
nursesPerHora.

blockingHours:- nurse(N), nurseIDandBlocking(N, S, F), findall(worksNH-N-H, hourInRange(S,F,H), Lits), 	exactly(0,Lits), fail.
blockingHours.

eachNurseOneType:- nurse(N), Lits = [nurseType-N-1, nurseType-N-2], exactly(1, Lits), fail.
eachNurseOneType.

eachNurseOneStart:- nurse(N), findall(startsNH-N-H, hour(H), Lits), exactly(1, Lits), fail.
eachNurseOneStart.

horariNurse:- nurse(N), type(T), hour(SH), workingHourForTypeAndStartH(T, SH, H),
writeClause([\+startsNH-N-SH, \+nurseType-N-T, worksNH-N-H]), fail.

horariNurse:- nurse(N), type(T), hour(SH), hour(H), \+workingHourForTypeAndStartH(T, SH, H),
writeClause([\+startsNH-N-SH, \+nurseType-N-T, \+worksNH-N-H]), fail.
